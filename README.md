<h1 align="center">CinioStreamBot</h1>
<p align="center">
...
<p align="center">
    <a href="https://github.com/Avipatilpro/FileStreamBot">
        <img src="https://i.ibb.co/ZJzJ9Hq/link-3x.png" height="100" width="100" alt="CinioStreamBot Logo">
    </a>
</p>
<p align='center'>
  CinioStreamBot provides stream links for Telegram files without the necessity of waiting for the download to complete, offering the ability to store files.
</p>
...
* `DATABASE_URL`: MongoDB URI #1. Saves User Data and Files List. `str`
* `DATABASE_URL_2`, `DATABASE_URL_3`, `DATABASE_URL_4`, ...: **(Optional, unlimited)** Add extra MongoDB URIs to auto-extend file storage. When DB 1 fills up (see `DB_MAX_SIZE_MB`), the bot automatically starts saving new files to DB 2, then DB 3, and so on — no code changes needed, just add another env var. `str`
* `DB_MAX_SIZE_MB`: Storage limit (in MB) per MongoDB shard before the bot rolls over to the next one. Default `460` (safe headroom under Atlas's 512MB free tier). `int`

#### 🌐 Optional Vars :

* `FQDN`: A Fully Qualified Domain Name without http/s. **Optional** — auto-detected on Render/Railway/Koyeb/Fly.io/Heroku/GitHub Codespaces from their own env vars, or falls back to your VPS's public IP if none of those are present. Set this yourself only if you want to override the detected value (e.g. behind your own reverse-proxy domain). `str`
...
- [**Me**](https://github.com/AvishkarPatil) : Owner of This CinioStreamBot
